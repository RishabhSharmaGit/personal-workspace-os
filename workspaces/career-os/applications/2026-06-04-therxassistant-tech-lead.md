---
slug: 2026-06-04-therxassistant-tech-lead
title: "TheRxAssistant — Tech Lead"
type: application
status: draft
tags: [healthcare-ai, founding, tech-lead, remote]
links: ["[[therxassistant-tech-lead]]", "[[master-resume]]"]
source: null
confidence: medium
created: '2026-06-04'
updated: '2026-06-04'
application_status: wishlist
company: therxassistant
target_role: therxassistant-tech-lead
channel: "[FILL — how did you find this? referral / board / inbound]"
region: remote-or-us
salary_band: "[FILL — not in JD]"
next_action_date: '2026-06-11'
---

## Timeline
- 04-Jun-2026 — JD received; target-role + tailored resume drafted. Status: wishlist (not yet applied).

## JD highlights
Founding-team Tech Lead at a healthcare-AI startup building AI agents for **medication discovery + access** (millions of users). Node.js/TS + PostgreSQL + generative AI/LLMs + AWS + healthcare-system integration. Strong leadership pedigree (Epocrates, One Medical, GoodRx, Spira). See [[therxassistant-tech-lead]] for full match analysis.

## Why I want this role
- Near-exact domain overlap (healthcare voice/AI + medication/Rx workflows = my Confido work)
- Founding-team, high-ownership, flat org — matches what I do well
- Strong investor/operator pedigree → credible outcome potential

## Concerns / red flags
- AWS-primary stack (I'm GCP-primary now — addressable)
- Salary band unknown; remote-from-India acceptability unknown
- Early-stage risk (counter: I'm already at a startup; known quantity)

## Tailored resume
`resumes/variants/2026-06-04-therxassistant-tech-lead.{html,md,pdf}`

## Next actions
- [ ] Confirm remote-from-India vs relocation expectation
- [ ] Find a warm intro if possible (check network against Epocrates/One Medical/GoodRx/Spira alumni)
- [ ] Finalize + send tailored resume + short cover note mirroring "medication discovery and access"
